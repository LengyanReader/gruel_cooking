...
- [submission/](submission/) keeps **derivatives** of `ai_and_math.md`, built for venues outside the repo and out of the parser's reach for the same reason as `ref1/`: [`submission/comment_draft.md`](submission/comment_draft.md) is a ~1,460-word English *Comment*-format rewrite with its Figure 1 dataset; [`submission/census_protocol.md`](submission/census_protocol.md) is the enumerable successor study that §11.5 of the essay calls for (frames, queries, endpoint, power, reliability gates). Both are **AI-drafted and unreviewed** (harness §I.6): nothing in them leaves the repo until a human author has accepted or deleted each claim.
