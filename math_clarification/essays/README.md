...
- [submission/](submission/) keeps **derivatives** of `ai_and_math.md`, built for venues outside the repo and out of the parser's reach for the same reason as `ref1/`: [`submission/comment_draft.md`](submission/comment_draft.md) is a ~1,650-word English *Comment*-format rewrite whose single permitted Figure 1 (regime staircase × verification heat-strip) is rendered by [`submission/checks/figure1.py`](submission/checks/figure1.py) from the draft's own dataset; [`submission/census_protocol.md`](submission/census_protocol.md) is the enumerable successor study that §11.5 of the essay calls for (frames, queries, endpoint, power, reliability gates), with its arithmetic and citation integrity checked by [`submission/checks/`](submission/checks/); [`submission/redteam_panel.md`](submission/redteam_panel.md) is an adversarial reviewer test of the draft's argument, distilled only from positions the essay itself reconstructs. All are **AI-drafted and unreviewed** (harness §I.6): nothing in them leaves the repo until a human author has accepted or deleted each claim.
